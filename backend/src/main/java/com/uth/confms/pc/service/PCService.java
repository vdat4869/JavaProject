...
public class PCService {
  ...
  public PCInvitationResponseDTO invitePCMember(PCInviteDTO dto, Long chairId) {
    ...
    if (pcInvitationRepository
        .findByConferenceIdAndInvitedUserId(dto.getConferenceId(), user.getId())
        .isPresent()) {
      throw new BusinessException("Invitation already sent to this user");
    }

    // Create invitation
    String token = UUID.randomUUID().toString();
    PCInvitation invitation =
        PCInvitation.builder()
  ...
  private void sendInvitationEmail(String email, String conferenceName, String token) {
    try {
      ...
      message.setTo(email);
      message.setSubject("PC Member Invitation - UTH-ConfMS");
      message.setText(
          String.format(
              "Xin chào,\n\n"
                  + "Bạn đã được mời làm thành viên Program Committee cho hội nghị: %s\n\n"
                  + "Vui lòng click vào link sau để chấp nhận lời mời:\n"
                  + "%s/pc/invitation/accept?token=%s\n\n"
                  + "Hoặc từ chối:\n"
                  + "%s/pc/invitation/decline?token=%s\n\n"
                  + "Link này có hiệu lực trong %d ngày.\n\n"
                  + "Trân trọng,\n"
                  + "UTH-ConfMS Team",
