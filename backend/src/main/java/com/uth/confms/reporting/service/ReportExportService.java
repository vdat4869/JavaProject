...
public class ReportExportService {
    ...
    private void addSubmissionsSheet(Workbook workbook, Long conferenceId, CellStyle headerStyle, CellStyle dataStyle) {
        ...
        for (Submission submission : submissions) {
            ...
            row.createCell(5).setCellValue(submission.getCreatedAt() != null ? submission.getCreatedAt().format(formatter) : "");
    ...
    private void addReviewsSheet(Workbook workbook, Long conferenceId, CellStyle headerStyle, CellStyle dataStyle) {
        ...
        int rowNum = 0;
        Row headerRow = sheet.createRow(rowNum++);
        String[] headers = {"Review ID", "Submission ID", "Reviewer ID", "Status", "Score", "Created At", "Submitted At"};
        for (int i = 0; i < headers.length; i++) {
            Cell cell = headerRow.createCell(i);
            cell.setCellValue(headers[i]);
            cell.setCellStyle(headerStyle);
        ...
        for (Long submissionId : submissionIds) {
            ...
            for (Review review : reviews) {
                ...
                row.createCell(5).setCellValue(review.getCreatedAt() != null ? review.getCreatedAt().format(formatter) : "");
                row.createCell(6).setCellValue(review.getSubmittedAt() != null ? review.getSubmittedAt().format(formatter) : "");
    ...
    private void addDecisionsSheet(Workbook workbook, Long conferenceId, CellStyle headerStyle, CellStyle dataStyle) {
        ...
        for (Long submissionId : submissionIds) {
            ...
            decisionRepository.findBySubmissionId(submissionId).ifPresent(decision -> {
                ...
                row.createCell(4).setCellValue(decision.getDecidedAt() != null ? decision.getDecidedAt().format(formatter) : "");
